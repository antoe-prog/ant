...
심사 계정임을 나타내는 사용자 속성이나 권한 분기는 두지 않는다. 메뉴, API 권한, 읽기·쓰기 가능 여부는 모든 계정과 동일하게 역할, `adminScope`, 배정 지점으로만 판단한다. 격리 지점의 `dataMode`는 공개 회원가입 대상에서 제외하는 데에만 사용한다.
...
두 검증은 역할별 로그인, 일반 RBAC와의 동등성, 합성 지점 쓰기, 실제 회원 비노출, 전역·교차 지점 쓰기 차단, 영문 안내문 길이, 비밀번호 비출력을 확인한다.
