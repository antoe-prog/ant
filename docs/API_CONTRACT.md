...
| 429 | `RATE_LIMITED` | 초대 비밀번호 시도 제한, `Retry-After` 포함 |
...
| `POST` | `/auth/logout` | 인증 또는 유효한 현재 기기 자격증명 | 예 | 세션을 폐기하고 현재 기기의 알림 연결을 해제한다. 로그인 세션이 이미 만료된 경우에도 기기 자격증명이 유효하면 해당 기기 구독만 해제한다 |
...
계정별 15분 내 로그인 비밀번호 실패 5회부터 내부 제한을 적용한다. 제한된 계정의 오입력도 미등록·대기·일반 오입력과 같은 `401 UNAUTHENTICATED` 본문을 반환하고 `Retry-After`를 노출하지 않아 계정 존재 여부를 구분할 수 없게 한다. 미등록·대기 계정과 없거나 손상된 비밀번호 해시도 더미 PBKDF2 검증을 한 번 수행한다. 차단된 재시도는 감사에는 남지만 제한 종료 시각을 연장하지 않으며 올바른 자격증명 로그인은 이전 실패 창을 초기화한다. 로그인·로그아웃·초대 수락·비밀번호·역할·계정 상태 변경은 같은 보안 잠금 아래 최신 상태를 다시 읽고 세션 발급·폐기를 저장한다. 무작위 세션 원문은 쿠키에만 한 번 제공하고 저장소에는 SHA-256 해시만 남긴다.
...
온라인 결제 요청은 예정/미납/만료 예정/부분 환불 결제에 대해 대표/총괄이 생성한다. `POST /api/v1/payments/{paymentId}/online-checkout`은 provider-neutral `onlinePayment` 메타를 결제에 저장하고 `payment.online_checkout.create` 감사 로그를 남긴다. 운영 PG/VAN 계약 전에는 `FINAL_JUDO_PAYMENT_PROVIDER`가 비어 있으면 mock provider로 동작하며, 설정된 값은 정확히 `external`이어야 한다. 오타·미지원 값은 `PAYMENT_PROVIDER_INVALID`로 준비를 차단한다. `FINAL_JUDO_PAYMENT_CHECKOUT_BASE_URL`이 없으면 앱 내부 리허설 URL을 만들며, 결제창 기본 주소를 설정하면 경로·쿼리·fragment·자격증명이 없는 HTTPS origin이어야 한다. 형식이 잘못되면 온라인 결제 요청을 저장하지 않고 `503 PAYMENT_RUNTIME_NOT_READY`로 차단한다.
...
`POST /api/v1/payments/webhook`은 `x-final-judo-payment-webhook-secret` 헤더를 상수 시간 비교로 검증한다. production의 `FINAL_JUDO_PAYMENT_WEBHOOK_SECRET`은 UTF-8 기준 32바이트 이상이고 예제·placeholder가 아니어야 한다. 누락되거나 정책을 충족하지 않으면 `503 PAYMENT_WEBHOOK_NOT_CONFIGURED`로 차단하고, 개발/테스트에서는 `final-judo-dev-webhook-secret` fallback으로 smoke 테스트를 수행한다. payload는 `{ "providerPaymentId": "...", "providerEventId": "...", "event": "paid" | "failed" | "refunded", "occurredAt": "2026-07-14T04:00:00.000Z", "amount": 30000, "receiptId": "...", "receiptUrl": "...", "reason": "..." }` 형태다. provider 결제·이벤트·영수증 식별자는 문자열이며 각각 160자 이하로 제한한다. `receiptUrl`은 2048자 이하의 자격증명 없는 HTTPS URL만 허용하고, 형식이 잘못된 메타데이터는 결제 상태·이력·감사 로그를 바꾸지 않고 `400 VALIDATION_ERROR`로 차단한다. `providerEventId`는 모든 이벤트에 필수인 전역 유일값이며 provider가 body가 아니라 header로 보낼 때는 `x-final-judo-payment-event-id`도 허용한다. body와 header에 ID가 모두 있으면 값이 정확히 같아야 하며 다르면 `400 VALIDATION_ERROR`로 차단한다. 같은 결제에 이미 처리한 ID는 상태 이력과 감사 로그를 추가하지 않고 `{ "duplicate": true }`로 응답하며, 다른 결제에 처리된 ID는 `409 PROVIDER_EVENT_CONFLICT`로 차단한다. `paid` 이벤트는 안전한 정수 원화 `amount`가 필수이고 최초 결제 요청 금액과 정확히 일치하지 않으면 `422 PAYMENT_AMOUNT_MISMATCH`로 차단한다. 최초 이벤트는 `occurredAt`을 생략할 수 있으며 이때만 서버 수신 시각을 사용한다. `occurredAt`을 보냈다면 최초·후속 여부와 관계없이 유효한 ISO 시각이어야 하고 잘못된 값은 `400 VALIDATION_ERROR`로 차단한다. 첫 이벤트 이후의 후속 이벤트는 `occurredAt`이 필수이며, 결제 요청 시각보다 5분을 초과해 이르거나 서버 수신 시각보다 5분을 초과해 미래이면 `400 EVENT_TIME_OUT_OF_RANGE`, 이미 처리한 최신 webhook보다 과거이면 `409 OUT_OF_ORDER`로 차단한다. 전액 환불·취소 상태와 환불 이력이 있는 결제는 `paid` 또는 `failed`로 역전할 수 없고 `409 INVALID_TRANSITION`을 반환한다. 성공 이벤트는 결제 상태를 `paid`로 바꾸고 receipt 메타를 저장하며, 실패 이벤트는 결제 상태를 유지하고 실패 사유를 `onlinePayment.failureReason`에 저장한다. 환불 webhook은 안전한 정수 원화 금액만 허용하고 `refundedAmount`, `refundReason`, `statusHistory`와 `payment.webhook` 감사 로그를 남긴다.
...
| `GET` | `/notifications/push-config` | 인증 | 아니오 | APNs/FCM/Web Push 제공자 준비 상태와 Web Push 공개키 조회. 활성 구독 수는 대표/총괄은 담당 지점 범위, 코치/회원/학부모는 본인 구독 범위로만 반환 |
| `POST` | `/notifications/subscriptions` | 인증 | 예 | 현재 기기의 Web PushSubscription 또는 APNs/FCM 네이티브 토큰을 둘 중 하나만 저장. Web endpoint/key, 네이티브 transport/device token, userAgent, `allowReactivation`을 검증한다. 다른 계정에 연결된 Web endpoint의 소유권을 이전할 때는 저장된 `auth`와 `p256dh`가 모두 일치해야 하며 불일치하면 `409 PUSH_SUBSCRIPTION_OWNERSHIP_CONFLICT`로 거부한다. 성공 시 30일 서버 만료가 포함된 기기별 불투명 자격증명을 HttpOnly 쿠키로 발급하고 서버에는 SHA-256 hash만 보관한다. 검증된 현재 기기의 endpoint/token이 회전하면 기존 구독 ID를 재사용하며, 새 endpoint 레코드가 이미 있으면 이전 레코드를 `token_rotated`로 비활성화하고 대기 발송을 취소해 활성 구독을 하나만 유지한다. 자동 앱 진입은 보안 변경으로 비활성화된 기기를 보존하고, 사용자 명시 동작만 재활성화한다. 기존 또는 교체 대상 구독의 제공자 호출이 진행 중이면 `409 PUSH_SUBSCRIPTION_UPDATE_PENDING`으로 보류한다 |
...
앱 알림함과 휴대폰 푸시는 같은 대상 산정 결과를 사용한다. 공지는 작성/발행되는 즉시 대상 회원/학부모 알림함에 미확인으로 표시된다. 서버는 공지, `dispatchState: requested` 감사 기록, 구독별 durable outbox 작업을 먼저 한 번에 저장한 뒤 응답하고, 외부 푸시는 응답 이후 작업으로 시작한다. 공지 발행·재발송·대회 신청 검토 경로는 최대 20건을 10개 worker로 처리하고, 예약 worker는 최대 50건을 10개 worker로 처리한다. 절대 상한은 실행당 100건·동시 10개이며 남은 작업은 outbox에 유지한다. 각 작업은 lease, revision, 지수 backoff, 최대 5회 시도, `sent`/`disabled`/`dead`/`cancelled` 상태를 가지며 응답 이후 전송에 실패하거나 실행 시간이 종료돼도 cron worker가 재처리한다. 발송 직전 현재 공지 내용·대상·사용자·지점·구독 활성 상태와 revision을 다시 확인한다. 공지 수정·삭제는 대기 작업을 취소하고 임대된 작업에는 취소 요청을 기록한다. 제공자 호출 전이면 발송을 중단하지만 호출이 시작된 뒤에는 취소를 보장할 수 없어 `deliveryMayHaveOccurred` 감사 상태로 남긴다. 제공자 호출은 15초 제한을 두며 시간 초과는 전송 결과 불확실 상태로 재시도한다. timeout은 호출자의 대기만 끝내고 provider Promise 중단을 보장하지 않으므로, 불확실 결과는 원래 lease 만료 시각까지 `providerFenceExpiresAt`을 보존한다. 이 보호 기간에는 같은 작업 재시도와 구독 소유권·키 변경을 차단하고 취소 후에도 전달 가능성을 유지한다. 재시도 임대는 보호 기간이 끝난 뒤 이전 시도의 provider 시작·완료·결과를 지우고 누적 전달 불확실성만 유지해, 새 provider 호출 중에도 구독 소유권·키 변경을 계속 차단한다. 이후 대상 변경으로 취소돼도 이전 시도의 `deliveryMayHaveOccurred`를 보존한다. 늦게 도착한 이전 revision 결과는 현재 작업 상태를 덮어쓰지 않고 거부 감사로 남긴다. 각 시도는 endpoint/key 없이 별도 `notification.dispatch` 감사 행으로 남긴다. Web Push는 제공자 전송 성공 직후 저장 장애에서 exactly-once를 보장할 수 없으므로 at-least-once이며, 재시도는 같은 `final-judo-notice-{noticeId}` tag를 사용해 표시 중복을 줄인다. `/branches/:branchId/notices/:noticeId/push`는 수동 재발송이며 16~128자 `Idempotency-Key`를 권장한다. 서버는 키 원문 대신 digest만 감사 기록에 남기고 같은 키 재시도는 기존 요청 결과를 재사용한다. 헤더가 없는 기존 호출은 5분 호환 구간으로 중복 요청을 합친다. 생성/재발송 응답은 `recipientCount`와 응답 시점의 outbox 상태를 반환한다. PWA 서버 푸시는 `FINAL_JUDO_VAPID_PUBLIC_KEY`, `FINAL_JUDO_VAPID_PRIVATE_KEY`, `FINAL_JUDO_VAPID_SUBJECT`를 사용한다. 키가 없거나 대상 기기 구독이 없으면 `configured: false` 또는 `attempted: 0`으로 알림함 표시와 휴대폰 푸시 미발송을 분리 안내한다. 여러 제공자 중 일부만 설정됐으면 설정된 대상은 발송하되 `unconfigured` 수와 `제공자 설정 미완료`를 실패 결과로 반환하고 감사 기록에 남긴다. 구독 저장소는 endpoint, `p256dh`, `auth`, 사용자, 지점 범위, 기기 자격증명 hash, 마지막 발송/실패/비활성화 시각을 보관한다. 원문 기기 자격증명은 HttpOnly 쿠키에만 두며 단순 구독 ID, 변조된 값, 서버 만료 30일이 지난 값으로는 로그아웃 또는 계정 전환 시 다른 구독을 해제할 수 없다. 같은 브라우저 endpoint가 다른 계정으로 연결되면 서버는 outbox와 같은 잠금 안에서 소유권을 한 계정으로 이전하고 변경 전후 사용자 ID를 감사 기록에 남긴다. 이전 계정의 provider 호출이 진행 중이면 전송 결과가 확정되거나 보호 기간이 끝날 때까지 `409 PUSH_SUBSCRIPTION_UPDATE_PENDING`으로 이전을 보류한다. 회원/학부모의 활성 알림은 항상 켜짐 정책을 유지하지만, 이 정책은 비밀번호·역할·지점 변경으로 폐기된 기기 자격증명을 다시 활성화하지 않는다. 자동 앱 진입은 기존 활성 endpoint의 계정 소유권만 동기화하고, 보안 폐기 기기는 사용자의 명시적 알림 활성화 요청이 있어야 재연결한다. 감사 로그에는 원문 endpoint/key를 남기지 않는다. provider가 404/410을 반환하면 현재 작업을 `disabled`로 정산하고 구독을 비활성화하며, 같은 구독의 나머지 대기·재시도 작업은 즉시 취소한다. 병렬 provider 호출이 이미 시작된 작업은 취소 요청과 `deliveryMayHaveOccurred` 경계를 유지한 채 결과를 정산한다. 기본 Vercel cron은 플랜 호환을 위해 일 1회이고, 50건을 넘는 backlog와 재시도는 다음 실행까지 남을 수 있다. 운영 push handoff는 `CRON_SECRET`으로 인증되고 현재 배포 플랜 또는 외부 스케줄러가 지원하는 30분 이내 retry worker 호출 증빙이 있어야 ready로 판정한다.

위 문단의 404/410 영구 실패 규칙은 Web Push에만 적용한다. APNs는 `BadDeviceToken`, `DeviceTokenNotForTopic`, `ExpiredToken`, `Unregistered`, FCM은 `UNREGISTERED`, `SENDER_ID_MISMATCH`와 FCM 전용 상세가 포함된 등록 토큰 `INVALID_ARGUMENT`만 기기 토큰 폐기 사유로 처리하고 구독에 `provider_invalid` 원인을 기록한다. APNs `BadPath` 같은 요청·구성 실패는 유효한 토큰을 비활성화하지 않고 재시도 또는 운영 확인 상태로 남긴다. 동일 payload로 복구할 수 없는 APNs `Forbidden`/`PayloadEmpty`/`PayloadTooLarge`와 일반 FCM `INVALID_ARGUMENT`은 기기 등록을 보존하면서 해당 outbox 작업만 첫 실패에서 `dead`로 종료한다. 일시적 실패 재시도는 로컬 지수 backoff, 전송 결과 불확실성 fence, provider의 `Retry-After` 중 가장 늦은 시각을 적용한다. FCM `429`는 provider 지시가 없거나 더 짧아도 최소 1분, APNs `5xx`는 최소 15분을 기다린다.

iPhone 푸시는 기본 24시간의 APNs store-and-forward 만료 시각을 사용한다. `FINAL_JUDO_APNS_TTL_SECONDS`로 60초~30일을 지정할 수 있고, 즉시 전달 한 번만 시도하고 저장하지 않을 때만 `0`을 명시한다. APNs 저장은 기기·앱별 최신 알림 하나만 보존할 수 있으므로 앱 알림함을 최종 기록으로 유지한다.

구독·감사 ID는 UUID 기반 런타임 ID를 사용한다. 구독 해제는 JSON 객체의 문자열 endpoint만 허용하며 객체·배열형 값은 활성 구독을 바꾸지 않고 `400 VALIDATION_ERROR`로 차단한다. 비활성화가 허용된 역할의 해지는 구독 레코드와 해당 구독에 연결된 outbox 작업을 한 잠금 안에서 함께 변경한다. 회원·학부모의 정상 활성 구독 해지 요청은 항상 켜짐 정책에 따라 `enforcedAlwaysOn: true`, `disabledAt: null`을 반환하고 차단 감사 기록을 남긴다. 보안 변경은 대상 사용자의 모든 푸시 구독을 `security_change` 사유로 비활성화하고 기존 비활성화 시각은 보존하면서 불투명 기기 자격증명 hash를 폐기한다. 보안 변경으로 이미 비활성화된 구독의 해지 요청은 저장 상태를 바꾸지 않고 기존 `disabledAt`, `enforcedAlwaysOn: true`, `reactivationRequired: true`를 반환한다. 기기 자격증명이 가리키는 활성 구독과 제출된 endpoint/token이 다르면 같은 기기의 provider 토큰 회전으로 처리한다. 새 endpoint가 없으면 기존 ID를 제자리 갱신하고, 이미 저장된 endpoint와 충돌하면 그 레코드를 유지하면서 이전 레코드를 비활성화한다. 두 경우 모두 원문 token은 감사 로그에 남기지 않는다.

비밀번호 재설정·재발급, 초대 수락·승인, 사용자 삭제·상태·역할·지점 변경과 구독 등록·해제는 계정 보안과 푸시 자격증명을 함께 바꿀 수 있으므로 공통 이중 잠금을 사용한다. 모든 경로는 `auth-security` 다음 `notification-outbox` 순서로 잠금을 얻어 교착을 방지하고, 보안 폐기와 구독 재연결 또는 provider 정산이 서로의 최신 상태를 덮지 못하게 한다. 보안 폐기와 비활성화가 허용된 직접 구독 해지는 대상 구독의 pending·retry 작업을 즉시 취소하고 leased 작업에는 취소 요청을 남긴다. provider 호출이 이미 시작된 작업은 단말 전달을 되돌릴 수 없으므로 `deliveryMayHaveOccurred` 감사 경계를 보존한다. 정확히 같은 활성 구독의 멱등 등록은 저장 없이 반환하지만, provider 호출이 진행 중인 구독의 소유권·key·재활성화·기기 정보 변경은 `409 PUSH_SUBSCRIPTION_UPDATE_PENDING`으로 보류한다.
...
stale revision으로 provider 결과 저장이 거부될 때도 worker가 보유한 작업과 최신 저장 작업의 `deliveryMayHaveOccurred`를 함께 계산한다. 현재 시도 결과가 확정 실패여도 이전 시도의 누적 전달 불확실성을 거부 감사에서 지우지 않는다.

하나의 발송 요청에서 일부 작업만 전송되고 나머지가 취소되면 요청 감사 결과를 성공으로 닫지 않는다. `sent`와 `cancelled` 수를 모두 보존하고 `dispatchState: blocked`와 확인 필요 메시지로 남겨 운영자가 부분 취소를 발송 완료로 오인하지 않게 한다.

`sent` 또는 `failed` 정산은 같은 lease와 revision에서 `beginPushDispatchProviderCall`로 provider 시작 시각과 fence를 먼저 저장한 작업만 허용한다. 시작 기록이 없는 결과는 `provider_call_not_started`로 거부하며, 발송 직전 대상 검증 실패처럼 실제 provider 호출이 없었던 `cancelled` 정산만 예외로 허용한다.

`cancelled` 정산도 저장된 provider 시작 시각을 기준으로 구분한다. 호출 전 취소는 `providerOutcome: not_started`이고 provider 완료 시각을 만들지 않는다. 호출이 시작된 뒤 결과를 확인하지 못한 취소는 `providerOutcome: uncertain`, `deliveryMayHaveOccurred: true`로 기록하고 원래 lease까지 bounded provider fence를 유지한다.
