...
- 회원 삭제 감사 스냅샷은 삭제된 회원의 실명·주소·생년월일·연락처와 자유 입력 삭제 사유 원문을 남기지 않고, 사유 확인 여부·지점·연령 구분·상태와 삭제된 운영 기록 건수만 2년간 보관한다. 2년 만료 시 DB 읽기·쓰기와 stale 병합 후 최종 검증, 승인된 일일 정리 경로에서 원본 저장소에 반영해 삭제하며, 법정 결제 원장의 5년 보유는 독립적으로 유지한다. 법정 원장의 보유일 형식이 손상된 경우 자동 삭제하지 않고 런타임 무결성 오류로 차단한다. 회원 삭제 감사시각 손상은 복구 가능한 blocker로 진단하며, 총괄의 명시적 무결성 복구가 보유 기준을 재설정하고 `system.integrity.repair` 기록을 남긴다.
