...
후속 지속 검토에서 회원 삭제 뒤 연결 계정의 세션·기기 구독·전송 중 푸시가 남는 P1 결함, 협회 제출 완료 대회 신청을 일반 상태 변경으로 되돌릴 수 있던 P1 결함, 참가 신청이 있는 대회 삭제로 신청 이력이 유실되던 P1 결함, 대한유도회 공식 목록에서 사라진 대회가 정상 일정으로 남아 신규 신청을 받던 P1 결함, 느린 이전 동기화가 나중 완료된 최신 동기화 결과를 덮어쓸 수 있던 P1 결함, 대한유도회 원본 중복·비정상 엔티티로 동기화가 불안정해지는 P2 결함을 추가로 수정해 누적 해결 건수는 66건이다.
