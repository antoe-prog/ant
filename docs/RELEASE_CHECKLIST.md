...
- [x] `npm run test:google-play-review-access`, `npm run test:google-play-review-api` 격리 지점·역할별 계정 5개, 사용자 심사 식별값과 앱 노출 전용 문구 부재, 일반 RBAC, 총괄의 배정 지점 쓰기와 실제 지점/회원/변경 기록 비노출, 전역·교차 지점 변경 차단, 프로비저닝 날짜 고정성과 iPhone/iPad·지역·TestFlight 비분기, 영문 500자 이내 Play Console 안내문, 비밀번호 출력 금지·`0600` 로컬 보고서 보관 통과
...
- [x] 동일 `test:runtime-retention-maintenance`가 회원 삭제 감사 기록 2년·분리 법정 거래기록 5년 경계, 정상 읽기의 원본 정리, stale 병합 후 재검증, 손상 보유일의 무음 삭제 차단, 관련 없는 감사 액션 보존과 반복 정리 멱등성을 검증
- [x] `npm run test:runtime-state-integrity` 손상된 회원 삭제 감사시각의 blocker 진단, 신규 손상 쓰기 차단, 총괄 명시 복구와 `system.integrity.repair` 감사기록 검증 통과
...
- [x] `npm run test:ios-capacitor-connection` iOS Capacitor 브릿지가 서명 앱 내부의 로그인·대시보드 UI를 열고, release config에 원격 `server.url`이 없으며 `CapacitorHttp`가 운영 API 요청을 담당하고 Simulator 실행을 IPA 배포 ready로 처리하지 않는지 검증 통과
- [x] `npm run test:ios-ipa-doctor` iOS IPA doctor가 운영 HTTPS API origin 누락을 blocked로 유지하고 API 전용 호스트를 로컬 UI의 정상 API origin으로 허용하며, App Store Connect profile과 기기 등록이 필요한 Development/Ad Hoc profile을 구분하고 release config fallback, strict 재실행, Local Profile Inventory의 `matching provisioning profile`과 등록 기기 포함 profile 수, UDID 원문 미기록을 JSON/Markdown에 남기는지 검증 통과
...
- [x] `npm run test:ios-provisioning-runbook` [docs/IOS_IPA_PROVISIONING_RUNBOOK.md](docs/IOS_IPA_PROVISIONING_RUNBOOK.md)가 Apple 계정 `roehf45@naver.com`, Team ID `CA7A5SP5G5`, bundle id `kr.co.finaljudo.multigym`, `FINAL_JUDO_IOS_API_ORIGIN`, 로컬 UI 번들·release `server.url` 제거, export method별 profile 요구사항, Xcode Download Manual Profiles, strict doctor/build 명령, UDID·비밀정보 미기록 규칙을 유지하는지 검증 통과
...
- [x] `npm run test:notification-push-handoff-draft` 운영 푸시 handoff 초안, APNs/FCM/Web Push 선택, 원문 private key 미기록, strict 통과 fixture 검증 통과
- [x] `npm run test:notification-push-handoff` 운영 푸시 handoff ready/blocked fixture, 선택한 APNs/FCM의 iOS/Android 실기기 HTTPS/provider URI 증빙, Web Push 활성 시만 VAPID 검증, `localhost`/`.example`/TODO production origin 차단, ISO 생성/승인 시각과 생성 이후 승인 순서, 원문 secret 차단 검증 통과
...
- [ ] 운영 APNs/FCM 설정 후 `npm run notification-push:handoff:draft -- --out=.data/notification-push-handoff.json`로 초안을 만들고 iOS/Android 실기기 권한·수신·클릭·공지 발송 변경 기록 HTTPS/provider URI 증빙, ISO 생성/승인 시각, 생성 이후 승인 순서를 채운 뒤 `npm run notification-push:handoff -- --file=.data/notification-push-handoff.json --out=.data/notification-push-handoff.report.json` 통과 및 증빙 보관. Web Push를 활성화하는 경우에만 VAPID 증빙을 추가한다
...
- [x] `npm run test:notification-outbox` 및 `npm run test:notification-outbox-integration` 구독별 durable 작업, lease/revision/backoff/dead letter, provider 시작 전 sent/failed 정산 거부, provider 호출 전·후 취소 결과의 `not_started`/`uncertain` 구분과 완료 시각·bounded fence 보존, 재시도별 provider fence 초기화·전송 중 소유권 변경 차단·누적 전달 불확실성 보존, timeout 뒤 원래 lease 만료까지 재시도·구독 소유권/키 변경을 막는 bounded provider fence, 404/410 만료 구독의 같은 기기 잔여 작업 취소와 병렬 provider 호출 불확실성 보존, 일부 발송·일부 취소 요청의 확인 필요 감사, 일부 제공자 설정 누락의 실패 요약, 수동 재발송 멱등 digest, 공지 변경 전송 취소 경계, provider timeout·불확실 전송·stale settlement 감사, 발송 직전 권한 재검증, 시도별 감사 기록, CRON_SECRET 보호 검증 통과
- [x] `npm run test:notification-push-handoff` 및 `npm run test:notification-push-handoff-draft`가 네이티브 운영 푸시에 `CRON_SECRET` secret-store 보관, 30분 이내 retry worker, 배포 플랜 또는 외부 스케줄러 지원, 실제 인증 호출 증빙을 요구하고 원문 secret 기록을 차단
...
- [ ] `.env.production.example`과 `docs/ENVIRONMENT_MATRIX.md` 기준으로 배포 플랫폼 secret store의 PostgreSQL, 결제, APNs, FCM 변수가 실제 운영값으로 채워졌는지 확인. Web Push를 사용할 때만 VAPID 변수를 추가로 확인
...
- 온라인 결제는 P1에서 provider-neutral 요청, webhook 상태 반영, provider event ID 중복 처리, 영수증 메타, CSV provider 컬럼, 코치 금액/결제 링크 마스킹까지 준비했다. 정기결제도 provider-neutral 약정 생성/해지, 다음 청구일, CSV 컬럼, 변경 기록, 코치 provider 약정 ID 마스킹까지 준비했다. 실제 PG/VAN 승인, 자동청구 billing key/mandate 보관 정책, 운영 provider event ID 필드/서명 매핑, 실 영수증 URL 검증은 운영 provider 확정 후 진행한다. 알림톡은 아직 제외 범위이며, P1 서버 푸시는 APNs/FCM/Web Push 제공자 상태, 기기 구독 저장/해지, 공지별 dispatch API와 제공자별 만료 토큰 비활성화까지 준비했다. 실제 기기 수신 검증은 운영 APNs/FCM secret, 실물 기기 권한·수신·클릭 증빙이 필요하며 Web Push를 사용할 때만 VAPID를 추가한다.
