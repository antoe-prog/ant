...
- [x] `npm run test:p1-operator-status` P1 readiness/Android doctor Markdown/iOS IPA doctor JSON/Markdown/action checklist/bundle/dispatch/GitHub connector access receipt/payload/대표 의사결정 등록표/evidence intake/release package/archive/storage receipt 기반 운영자 상태판, Markdown GitHub Connector 섹션(repo/checkedAt/default branch/권한/publish 상태), iOS Local Profile Inventory 섹션(profile 파일 수, Team ID/bundle id 매칭 수, 등록 기기 포함 profile 수, UDID 원문 미기록), Deferred External Prep 섹션(운영 웹앱 origin, iOS 실제 iPhone/provisioning profile, 사용자 보류, readiness blocked 유지), Markdown Owner Decision Register 섹션, Markdown Release Custody 섹션(prerequisite/package/archive/storage receipt/next action), readiness/evidence intake blocked 상태에서 release custody prerequisite sequencing, 담당 영역별 `externalBlockers`, dispatch/issue registration/storage receipt 한국어 next action, publish/보관 가능 여부, publishable payload의 access receipt 필수 조건, Android doctor Markdown 누락/형식 불일치, iOS IPA doctor blocked/누락, storage receipt placeholder, secret-like 값 차단 fixture 통과
...
- [x] `npm run test:payment-checkout-method-flow` 성인 회원/학부모 결제 상세의 결제자 정보, 무통장입금/신용카드/가상계좌/계좌이체 선택, 카드사 그리드, 우리WON페이 모달, 학부모 자녀 결제 화면이 390px 모바일에서 overflow/콘솔 오류 없이 동작하고 실제 PG/API 호출이 없는지 검증 통과
...
- [x] `npm run test:guardian-age-policy-ui` 운영자 회원 상세 연령 정책과 보호자 연결 후보 렌더링 검증 통과
- [x] `npm run test:admin-user-guardian-bottom-safe-area` 관리자 사용자 상세 보호자 연결 편집 화면의 모바일 하단 내비 안전 여백 검증 통과
...
- [x] `npm run test:p5-p10-internal-readiness` P5~P10 내부 readiness audit 통과 대상: 앱 UI 내부 release/audit 카드 비노출, `.data/p5-p10-internal-audit.json`, `.data/p5-p10-internal-audit.md`, `.data/mobile-builds/ios/p5-p10-simulator-screenshots` 관리자 설정/코치/회원/학부모 캡처 4개, `.data/mobile-builds/ios/visible-text-audit-20260621-recheck` 관리자 변경 기록/코치 회원 화면 recheck 캡처, P1 외부 blocker blocked 7/7, Android/iOS release blocker, 코치 모바일 저장 상태 패널 safe area 배치와 기본 상태 한 줄 요약, 코치 회원 화면 최근 상담/주의 메모 1건 요약과 빈 주의사항/빈 메모 문구 비노출, Next 개발 표시 배지 비활성화, 역할별 `autoLogin` deep link 실제 서비스 화면 진입, 로그인 `quickLogin=1` hydration mismatch/빈 화면 방지, 예시 이메일 placeholder와 `010-0000-0000` 같은 더미 연락처 placeholder, 로그인 클라이언트의 `.test` seed 이메일/공통 비밀번호/계정 채우기 UI 및 직접 접근 차단/사용자 목록/초대/공지 작성 화면의 내부 권한·지점 범위 표현 비노출, README/QA/릴리즈/백로그 문서 정합성. `npm run p5-p10:internal-audit`는 감사 산출물만 재생성한다. 이 내부 audit는 출시 완료, IPA ready, 운영 ready가 아니며 iOS Simulator 성공을 IPA ready로 보지 않음
- [x] `npm run test:notice-delete-ui` 공지 삭제 권한, 삭제 후 목록/알림 상태 회귀 검증 통과
- [x] `npm run test:visible-app-copy-stability` 390px 모바일 실제 화면에서 로그인/회원가입/비밀번호 재설정/초대 수락/계정 전환 진입 화면과 역할별 앱 화면이 FINAL 벡터 로고, 필수 폼/계정 전환 버튼, 콘솔 에러 없음, 더미/샘플/테스트 계정/릴리즈 내부 문구 비노출, 가로 overflow 없음, runtime error 없음 상태를 유지하고 회원/학부모 계정 화면은 불필요한 활성 상태 카드 없이 계정 전환/로그아웃 44px 액션을 검증 통과. 관리자 사용자 목록 수정/삭제/비밀번호 재발급 액션은 compact 세로 스택으로 보이며 수정 폼의 선택 비밀번호 입력 2개와 수정/삭제/재발급 폼 접힘/토글 상태를 검증한다. 대표/총괄 `/app/notices`는 공지 목록과 읽음 처리가 먼저 보이고 `공지 작성` 폼은 44px `작성 열기` 토글 뒤에 접힌 상태로 유지한다. 코치 회원 화면은 최근 메모 1건만 기본 노출하고 44px 더보기로 확장되며 빈 주의사항/빈 상담 문구를 반복하지 않는다. 코치 수업 화면은 내부 P3 운영 패널과 코드형 마감 문구가 실제 앱 DOM에 노출되지 않음을 확인한다. 대표 리포트는 지점별 회원/출석/위험/매출 그래프와 내부 P3 운영 패널/코드형 운영 확인 문구 비노출을 확인하고, 대표 내 지점은 지점별 회원 유지/수업 채움/결제 위험 그래프와 44px 액션 버튼을 확인하며, 회원 `/app/notifications` alias는 공지 필터/읽음 액션을 함께 확인
...
- [x] `npm run test:ios-ipa-doctor` iOS IPA doctor가 운영 origin 누락 또는 API-only `api.*` origin 입력 시 blocked를 유지하고 Apple Developer iPhone UDID 등록, Xcode 수동 profile 다운로드, `mobile/ios/release-config.json` 기본 Team ID/bundle id fallback, `APPLE_TEAM_ID`/`FINAL_JUDO_IOS_SERVER_URL`, strict 재실행, IPA build 힌트, Local Profile Inventory, UDID 원문 미기록을 JSON/Markdown에 남기는지 검증 통과
- [x] `npm run ios:ipa:doctor -- --out=.data/mobile-builds/ios/ios-ipa-doctor.json --markdown=.data/mobile-builds/ios/ios-ipa-doctor.md` iOS IPA 로컬 빌드 준비 상태 JSON/Markdown 확인. 운영 HTTPS 웹앱 origin, Capacitor iOS 프로젝트, Xcode/simctl, Apple Team ID, code signing identity, 등록된 iPhone/provisioning profile을 점검하고 provisioning 해소 힌트와 profile inventory를 남겨 Simulator 성공과 IPA 배포 가능 상태를 구분
- [x] `npm run test:ios-provisioning-runbook` [docs/IOS_IPA_PROVISIONING_RUNBOOK.md](docs/IOS_IPA_PROVISIONING_RUNBOOK.md)가 Apple 계정 `roehf45@naver.com`, Team ID `5GWZ792DWH`, bundle id `kr.co.finaljudo.multigym`, `FINAL_JUDO_IOS_SERVER_URL`, 실제 iPhone UDID 등록, matching provisioning profile, Xcode Download Manual Profiles, strict doctor/build 명령, UDID 원문 미기록 규칙을 유지하는지 검증 통과
...
- [ ] iOS IPA 생성 전 [docs/IOS_IPA_PROVISIONING_RUNBOOK.md](docs/IOS_IPA_PROVISIONING_RUNBOOK.md)에 따라 `/login`과 `/app/dashboard`를 서빙하는 실제 운영 HTTPS 웹앱 origin, Apple Developer 실제 iPhone UDID 등록, Team ID `5GWZ792DWH`/bundle id `kr.co.finaljudo.multigym` matching provisioning profile, Xcode Download Manual Profiles 설치를 완료한 뒤 `APPLE_TEAM_ID=5GWZ792DWH FINAL_JUDO_IOS_SERVER_URL=<운영 HTTPS 웹앱 origin> npm run ios:ipa:doctor -- --team-id=5GWZ792DWH --strict --out=.data/mobile-builds/ios/ios-ipa-doctor.json --markdown=.data/mobile-builds/ios/ios-ipa-doctor.md`와 `npm run ios:ipa:build -- --team-id=5GWZ792DWH --xcode-export-method=release-testing --allow-provisioning-updates` 순서로 실행. API-only `api.*` origin과 UDID 원문은 보관하지 않는다
...
- [x] `npm run test:team-agent-prompts` [docs/TEAM_AGENT_PROMPTS.md](docs/TEAM_AGENT_PROMPTS.md)가 P0 완료 후 P1 운영/모바일 배포 단계 기준의 6인 팀 목표 프롬프트를 유지하고 Android APK artifact/iOS Simulator와 IPA ready 분리, Team ID/bundle id, 7개 외부 blocker, P1 필수 검증 명령을 누락하지 않는지 검증 통과
...
- [x] `/app/admin/settings`의 `P1 최종 배포 준비` 패널에서 운영 배포, Android release, 결제 provider, 운영 푸시, P1 handoff issue registration receipt, 파일럿 최종 status 리포트, 담당 영역별 외부 증빙 입력 지도, 모바일 앱 배포 상태, `npm run test:ios-capacitor-connection`, `npm run ios:ipa:doctor`, [docs/IOS_IPA_PROVISIONING_RUNBOOK.md](docs/IOS_IPA_PROVISIONING_RUNBOOK.md) iOS IPA provisioning 런북과 `npm run test:ios-provisioning-runbook`, 외부 증빙 수집 확인, 증빙 형식 가드, `.data/postgres-docker-readiness.md` Postgres/Docker doctor, `.data/mobile-builds/ios/ios-ipa-doctor.md` iOS IPA doctor Markdown, [docs/P1_OWNER_PROGRESS_REPORT.md](docs/P1_OWNER_PROGRESS_REPORT.md) 대표 진행 보고서와 `npm run test:owner-progress-report`, `.data/p1-owner-progress-report.md` 대표 진행 보고서 초안과 `npm run owner:progress-report:draft`, `.data/p1-owner-decision-register.*` 대표 의사결정 등록표와 `npm run owner:decision-register`, `.data/p1-owner-decision-register.completed.*` completed 등록표와 `npm run owner:decision-register:apply-csv`, `.data/p1-owner-briefing-package.json` 대표 공유 패키지와 `npm run owner:briefing-package`, 운영자 상태판 Markdown GitHub Connector/Release Custody 섹션 안내, 초안 workspace/readiness 감사/최종/package 보관 명령 확인 가능. iOS는 Simulator 성공과 IPA 배포 가능 상태를 구분하며, 등록된 iPhone/provisioning profile이 준비되기 전까지 IPA ready로 표시하지 않는다.
- [x] 같은 패널의 `보류 중인 외부 준비` 섹션에서 운영 웹앱 origin 확정과 iOS 실제 iPhone/provisioning profile을 `사용자 보류`로 분리해 표시한다. 이 보류 항목은 내부 스냅샷/문서/테스트 갱신을 막지 않지만 `/login`과 `/app/dashboard`를 여는 운영 웹앱 origin 및 profile inventory 0 해소 전까지 출시 판단은 보류로 유지한다.
...
- [x] `/app/admin/settings`는 운영 확인 화면만 유지하고 P4 내부 상태판은 표시하지 않는다. `npm run test:p4-simulator-rehearsal`은 `.data/mobile-builds/ios/p4-simulator-screenshots` 증거와 P1 readiness blocked 7/7을 확인하며, iPhone 16e Simulator 캡처는 실제 서비스 화면 연결 증거로 보관하지만 iOS Simulator 성공을 IPA ready로 보지 않음 상태를 유지한다.
