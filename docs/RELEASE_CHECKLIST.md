...
- [x] `npm run test:google-play-review-access`, `npm run test:google-play-review-api` 격리 지점·역할별 계정 5개, 사용자 심사 식별값과 앱 노출 전용 문구 부재, 일반 RBAC, 총괄의 배정 지점 쓰기와 실제 지점/회원/변경 기록 비노출, 전역·교차 지점 변경 차단, 영문 500자 이내 Play Console 안내문, 비밀번호 출력 금지·`0600` 로컬 보고서 보관 통과
