    ...
    <ScreenContainer>
      <View className="px-5 pt-4 pb-3 flex-row items-center justify-between">
        ...
        {isManager && (
          ...
          >
            <Text className="text-white font-semibold text-sm">+ 공지 작성</Text>
          </TouchableOpacity>
        )}
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View className="items-center py-12">
              ...
              <Text className="text-muted text-center">공지사항이 없습니다</Text>
            </View>
          }
          renderItem={({ item }) => (
            <TouchableOpacity
                ...
                <View className="flex-row items-start justify-between gap-2">
                  <View className="flex-1">
                    <View className="flex-row items-center gap-2 mb-1">
                      {!item.readAt && (
                        <View className="w-2 h-2 rounded-full mt-1.5" style={{ backgroundColor: "#1565C0" }} accessibilityLabel="미읽음" />
                      )}
                      {(item as { isPinnedEffective?: boolean }).isPinnedEffective ? (
                        <View className="px-2 py-0.5 rounded-full" style={{ backgroundColor: "#1565C020" }}>
                          <Text className="text-xs font-semibold" style={{ color: "#1565C0" }}>📌 고정</Text>
                        </View>
                      ) : null}
