// 개발 전용 테마/컬러 팔레트 시각 확인 화면.
// 프로덕션 빌드에서는 아래 early-return으로 화면이 비어 있고, Metro의 dead-code
// 제거가 작동해 내부 구현 쪽 번들 크기 영향을 최소화한다. 파일 존재 자체는 라우팅된다.
...
export default function ThemeLabScreen() {
  // 프로덕션 빌드에서는 진입해도 아무 UI도 렌더하지 않음.
  // 개발자가 직접 URL을 열거나 Dev 메뉴에서만 접근.
  if (!__DEV__) {
    return (
      <ScreenContainer className="flex-1 items-center justify-center p-6">
        <Text className="text-muted">Dev-only screen.</Text>
      </ScreenContainer>
    );
  }
