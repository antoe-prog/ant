...
export default function QrCodeScreen() {
  ...
  // 서버/스캐너가 허용하는 범위(5분 단위 × ±3) 안에 항상 현재 시각을 유지하도록 30초마다 tick.
  const [tickSec, setTickSec] = useState(() => Date.now());
  useEffect(() => {
    const timer = setInterval(() => setTickSec(Date.now()), 30_000);
    return () => clearInterval(timer);
  }, []);

  // QR 코드에 인코딩할 데이터: 회원 ID + 이름 + 타임스탬프(5분 버킷)
  const qrPayload = myProfile
    ? JSON.stringify({
        memberId: myProfile.id,
        name: myProfile.name,
        t: Math.floor(tickSec / (5 * 60 * 1000)),
      })
    : "";
