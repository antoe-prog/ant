...
export default function AttendanceScreen() {
  ...
  // 출석 변경 시 같이 무효화해야 하는 쿼리 묶음 (오늘/타임라인/홈 대시보드 카드·차트).
  ...
  const handleBarCodeScanned = ({ data }: { type: string; data: string }) => {
    ...
    let payload: { memberId?: number; name?: string; t?: number };
    try {
      payload = JSON.parse(data) as { memberId?: number; name?: string; t?: number };
    } catch {
      console.warn("[QR] parse failed:", data?.slice(0, 80));
      setQrError("QR 코드를 읽을 수 없습니다.");
      return;
    }

    if (!payload.memberId || typeof payload.memberId !== "number") {
      console.warn("[QR] missing memberId:", payload);
      setQrError("유효하지 않은 QR 코드입니다.");
      return;
    }

    // 타임스탬프 유효성 검사: 허용폭을 5분 버킷 ±4(≈20분)로 넓혀서 시계 스큐·느린 관리자 반응까지 흡수.
    if (payload.t !== undefined) {
      const currentT = Math.floor(Date.now() / (5 * 60 * 1000));
      if (Math.abs(currentT - payload.t) > 4) {
        console.warn(`[QR] expired: delta=${currentT - payload.t}`);
        setQrError("QR 코드가 만료되었습니다. 회원에게 QR 화면을 새로고침하라고 알려주세요.");
        return;
      }
    }

    // mutation 성공/실패 결과에 따라 UI를 바꾸므로, 결과 세팅은 mutate 직전 한 번만.
    ...
    <ScreenContainer>
      ...
      {loadingToday ? (
        ...
        <FlatList
          ...
          {...listPerfProps}
          ListEmptyComponent={<EmptyState emoji="📋" title="오늘 출석한 회원이 없습니다" subtitle="우측 상단에서 회원 선택 후 체크인하세요." />}
          renderItem={({ item }) => (
          ...
          {qrResult ? (
            ...
            <View style={styles.qrResultContainer}>
              {checkByQrMutation.isPending ? (
                ...
                <View style={styles.qrResultCard}>
                  {checkByQrMutation.data?.isNew === false ? (
                    <>
                      <Text style={styles.qrSuccessIcon}>ℹ️</Text>
                      <Text style={[styles.qrSuccessTitle, { color: "#1565C0" }]}>이미 출석 등록됨</Text>
                      <Text style={styles.qrSuccessName}>{qrResult.name}</Text>
                      <Text style={styles.qrSuccessDate}>
                        {formatDate(checkByQrMutation.data?.attendanceDate ?? TODAY)} 이미 출석 처리되어 있습니다.
                      </Text>
                    </>
                  ) : (
                    <>
                      ...
                      <Text style={styles.qrSuccessName}>{qrResult.name}</Text>
                      <Text style={styles.qrSuccessDate}>
                        {formatDate(checkByQrMutation.data?.attendanceDate ?? TODAY)} 출석 기록됨
                      </Text>
                    </>
                  )}
      ...
      <Modal visible={showCheckIn} animationType="slide" presentationStyle="pageSheet">
        <View className="flex-1 bg-background">
          ...
          <FlatList
            ...
            contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 20, gap: 8 }}
            {...listPerfProps}
            ListEmptyComponent={<EmptyState emoji="🎉" title="모든 회원이 출석했습니다" />}
            renderItem={({ item }) => {
