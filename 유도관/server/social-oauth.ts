 ...
 *   - 웹 관리자: `?return=webadmin` → 콜백 후 `WEB_ADMIN_POST_LOGIN_URL#session=JWT` (미설정 시 Expo Web)
 *
...
export function registerSocialOAuthRoutes(app: Express) {
  ...
  app.get("/api/oauth/google", (req: Request, res: Response) => {
    ...
    const state = ret === "webadmin" ? "webadmin" : (req.query.state as string) || "web";
  ...
  app.get("/api/oauth/google/callback", async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (state && state.startsWith("manus")) {
        const user = await getUserByOpenId(userInfo.openId);
        res.json({ app_session_id: sessionToken, user: buildUserResponse(user) });
      } else if (state === "webadmin") {
        const base = (process.env.WEB_ADMIN_POST_LOGIN_URL || getFrontendUrl()).replace(/\/$/, "");
        res.redirect(302, `${base}#session=${encodeURIComponent(sessionToken)}`);
      } else {
        res.redirect(302, getFrontendUrl());
      }
    } catch (err) {
