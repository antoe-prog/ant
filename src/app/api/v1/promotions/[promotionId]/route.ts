...
export async function PATCH(
  ...
  return withServerDbLock(finalPromotionStateLockKey, async () => {
    ...
    const isExamDecision = result === "passed" || result === "failed";

    if (isExamDecision && !hasFinalPromotionExamDateArrived(promotion.examDate, formatDateKey(new Date()))) {
      return jsonError(422, "BUSINESS_RULE_FAILED", "정책 심사일이 지난 뒤에만 결과를 확정할 수 있습니다.");
    }

    if (isExamDecision && !isExactNextCompatiblePromotionBelt(member.belt, promotion.toBelt)) {
      return jsonError(409, "CONFLICT", "현재 띠의 정확한 다음 단계가 아닌 심사는 확정할 수 없습니다.");
    }
