...
export async function PATCH(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => null)) as PilotReadinessUpdateBody | null;
  ...
  const targetCheck = db.pilotReadinessChecks.find((check) => check.id === checkId);
  ...
  const nextDb = await writeServerDb({
    ...db,
    pilotReadinessChecks: db.pilotReadinessChecks.map((check) => (check.id === checkId ? nextCheck : check)),
