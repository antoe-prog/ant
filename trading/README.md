...
닷컴 구간에서만 도움이 됐고(MDD 19.9% → 13.0%), 금융위기에서는 재앙이었다
(+18.3% → +0.3%). 전구간으로도 수익 65.1% → 43.6%, Sharpe 0.46 → 0.37.

이유는 분명하다. **SMA 크로스가 이미 추세 이탈 시 빠져나가므로 국면 필터가
하는 일이 겹친다.** 겹치는 것에 더해 재진입까지 늦추니, 막아주는 손실보다
놓치는 반등이 크다.

→ **추세추종 전략에는 국면 필터를 켜지 말 것.** 값어치가 있다면 추세를 스스로
보지 않는 전략(평균회귀 등)과 조합할 때다. 그건 아직 측정하지 않았다.
