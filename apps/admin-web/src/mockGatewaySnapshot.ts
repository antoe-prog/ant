/**
 * 게이트웨이가 꺼져 있을 때 Admin UI만 확인할 때 쓰는 스냅샷.
 * `services/gateway/app/product_vision.py` 와 맞출 것(SSOT 변경 시 여기도 수정).
 */
export const mockHealth: Record<string, unknown> = {
  status: "ok",
  _demo: true,
};

export const mockProductVision: Record<string, unknown> = {
  one_liner:
